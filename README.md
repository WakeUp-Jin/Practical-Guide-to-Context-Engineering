...
本项目的目标，是为开发者和研究者提供一份大模型应用开发的骨架思路，以上下文组成作为大模型应用开发的核心，有关大模型应用开发的技术都可以借助上下文工程这个概念互相联系起来
...
## 其他
- [更新日记](./docs/更新日记/更新日记.md): 记录着项目更新的内容进度

## 📖 内容导航
### 前言
- [前言-从零到一：基于上下文工程的 Agent 后端设计](./docs/前言/从零到一：基于上下文工程的%20Agent%20后端设计.md): 从零到一：基于上下文工程的 Agent 后端设计 ✅

### 第一部分：上下文工程基础技术
<!-- - [第一章 数据持久化](./docs/)：介绍传统数据库（MySQL、MongoDB、Redis 等）在大模型应用中的作用：状态存储、用户数据、日志记录  -->
- [第一章 向量存储和嵌入模型](./docs)：语义检索的底层原理、嵌入模型的选择与评测、主流向量数据库的特性 
- [第二章 知识图谱](./docs)：知识的结构化表达、实体关系抽取、知识图谱与向量检索的互补 
- [第三章 RAG策略](./docs/RAG技术/RAG策略-index.md)：从检索增强生成（RAG）的流程拆解到优化方法，涵盖索引构建、检索策略、融合方式 ✅
- [第四章 搜索代理](./docs/搜索代理/搜索代理.md)：搜索代理的原理、实现方式、优化方法 ✅
...
- [第十章 相关上下文模块](./docs)：如何动态选择与任务最相关的上下文（检索、排序、过滤、融合）
  - [ Harness Engineering理解 ](./docs/相关上下文模块/Harness%20Engineering理解.md) ✅
...
- [第十四章 上下文压缩：ClaudeCode、Gemini 与工具消息裁剪](./docs/上下文管理/上下文压缩：ClaudeCode、Gemini与工具消息裁剪.md)：上下文压缩的原理、实现方式、优化方法 ✅

### 第四部分：Agent架构设计
- [第十五章 两种世界的交互形态：协同Agent与自主Agent](./docs/Agent形态/两种世界的交互形态：协同Agent与自主Agent.md)：Agent的两种形态：协同Agent与自主Agent以及开发策略 ✅
- [第十六章 智能体系统构建策略：单智能体和多智能体](./docs/Agent形态/智能体系统构建策略-单智能体和多智能体.md)：智能体系统构建策略：单智能体和多智能体以及开发策略 ✅
...
### 第六部分：AI协作编码与上下文工程
- [第二十章 Anthropic 黑客马拉松冠军- ClaudeCode配置整理和补充](./docs/AI协作编码与上下文工程/Anthropic%20黑客马拉松冠军-%20ClaudeCode配置整理和补充.md)：Anthropic 黑客马拉松冠军- ClaudeCode配置整理和补充 ✅


### 第七部分：上下文工程实践项目
- [首页：ReasonCode项目介绍](./docs/ReasonCode开发设计文档/首页：ReasonCode项目介绍.md)✅
